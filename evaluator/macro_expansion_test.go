...
func TestDefineMacros(t *testing.T) {
	...
	if len(program.Statements) != 2 {
		t.Fatalf("Wrong number of program.Statement. got%d", len(program.Statements))
	}
	...
	if macro.Parameters[0].String() != "x" {
		t.Fatalf("parameter is not 'x' got %T", macro.Parameters[0])
	} 
	if macro.Parameters[1].String() != "y" {
		t.Fatalf("parameter is not 'y' got %T", macro.Parameters[1])
	} 
...
func TestExpandMacros(t *testing.T) {
	tests := []struct {
		...
		{
			`
			let reverse = macro(a, b) { quote(unquote(a) + unquote(b));};
			reverse(2 + 2, 10 - 5);
			`,
			`(2 + 2) + (10 - 5)`,
		},
